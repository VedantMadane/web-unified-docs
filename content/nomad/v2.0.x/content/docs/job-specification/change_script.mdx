...
created_at: 2026-02-16T16:53:49.000Z
last_modified: 2026-02-16T16:53:49.000Z
...
- `run_on_first_render` `(bool: false)` - If `true`, the configured script will also be executed after the initial template render (when the task first starts and the template content is first available), in addition to on subsequent re-renders. This is useful for running initialization or reload logic that depends on the template having been rendered at least once.
